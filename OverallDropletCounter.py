...
def main():
	...
	#Threshold the greyscale bf image and smoothen
	ret,thresh_img = cv2.threshold(gs_bf_image, 100, 255, cv2.THRESH_BINARY)
	thresh_img = cv2.medianBlur(thresh_img, 5)
	
	#eroding via Morphology Ex to random bright spots
	kernel = np.ones((3,3), np.uint8)
	opening = cv2.morphologyEx(thresh_img, cv2.MORPH_OPEN, kernel, iterations =2)
	
	#smoothen out to remove any 1 pixel remnants
	...
	cv2.waitKey()
	cv2.imwrite('coloredArt.tif', fitc_image)
